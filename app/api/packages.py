...
@router.get("/all-packages", response_model=AllPackagesListResponse)
async def get_all_packages(
    tenant_id: uuid.UUID = Depends(get_current_tenant_id),
    db: Session = Depends(get_db),
):
    """
    Get all packages for the current tenant.
    Requires X-Tenant-Key header.
    """
    packages = PackagesService.list_packages(db, tenant_id=tenant_id)
    return {
        "success": True,
...
@router.get("/detail/{package_id}", response_model=PackageDetailResponse)
